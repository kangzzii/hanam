       ...
       <div id="a2_content">
           <form name="d5frm" method="post" action="d5_ok.php"> 
                <span style="font-size: 1.3m;font-weight: bold;">
                    1:1 문의  <input type="button" value="문의하기" 
                            onclick="location.href='d5.php'"
                            style="background-color:#6485c0;float:right;border: none; margin-bottom: 30px;"
                            class="c2_bt">
                </span><br><br>
                <table class="d5_content_table">
                    <tr>
                        <th>이름</th>
                        <th>제목</th>
                        <th>답변여부</th>
                ...
                <?php
                $sql="select * from hanam_qna";
                $rs=mysqli_query($conn,$sql);
                while($row=mysqli_fetch_array($rs)){
                ?>
                    <tr>
                        <td><a href="qna_checkpw.php?no=<?php echo $row["no"]?>"><?php echo $row["name"]?></a></td>
                        <td><a href="qna_checkpw.php?no=<?php echo $row["no"]?>"><?php echo $row["title"]?></a></td>
                        <td>
                       <?php $sql2="select count(*) as cnt from hanam_qnare where p_no=$row[no]";
